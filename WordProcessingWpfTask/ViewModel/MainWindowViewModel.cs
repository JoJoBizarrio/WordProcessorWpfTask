...
namespace WordProcessingWpfTask.ViewModel
{
	public class MainWindowViewModel : ViewModelBase
	{
		...
		public IAsyncCommand<object> OpenAsync
		{
			get
			{
				...
				return _openAsync = new AsyncCommand<object>(async obj =>
				{
					...
					{
						var temp = new TextFile() { FilePath = filePath };
						_redactor.ReadAsync(temp, filePath, 0);

						TextFilesCollection.Add(temp);
					}
				});
			}
		}
		...
		public ICommand UpdateText => _updateText ??= new RelayCommand(obj => { _redactor.ReadAsync(SelectedTextFile, SelectedTextFile.FilePath, (double)obj); });
	}
}