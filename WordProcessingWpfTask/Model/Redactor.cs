...
namespace WordProcessingWpfTask.Model
{
    public class Redactor : IRedactor
    {
        ...
        async public Task<TextFile> OpenFileAsync(string path)
        {
            ...
            var fileStream = new FileStream(path, FileMode.Open, FileAccess.Read, FileShare.None, 4096, true);
            var sr = new StreamReader(fileStream);
            //sr.
            var buffer = new byte[(int)fileStream.Length];
            var task = await fileStream.ReadAsync(buffer, 0, (int)fileStream.Length);
            return new TextFile() { Title = Path.GetFileNameWithoutExtension(path), Text = ASCIIEncoding.ASCII.GetString(buffer), FilePath = path };

            //var bufferReader = new BufferedStream(fileStream);
            //bufferReader.ReadAsync(buffer);
            //using (var reader = new BufferedStream(new FileStream(path, FileMode.Open, FileAccess.Read), 64))
            //{
            //    var text = await reader.ReadAsync(new byte[10]);
        ...
        async public Task ReadAsync(TextFile textFile, string path, int startPos, int count)
        {
